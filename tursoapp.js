...
const config = {
  url: 'libsql://apartment-qaver.aws-ap-south-1.turso.io',
    authToken: 'eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3NjM2MzgwMjksImlkIjoiOWJlYjRlZDQtNTMxOS00MjY2LTk5M2ItY2YyNzgwOTc0NjgzIiwicmlkIjoiNWM2ZDk0YjEtMzFkYi00YmI5LTlkM2QtZjQzZjY1YWIzZDdhIn0.LzkS6WxZI-3hNoFgKKeneN-o81Gw0owzm3Rg951KtVr25My5MP2e5sVH4g10wjoV6l0w4CahWRaec632xBMkAQ'
};
...
/*async function runQuery()
{
    try 
    {
        ...
        const rs = await db.execute("SELECT * FROM mast1 where id = 51");
        console.log(rs.rows);
    } 
    catch (e) 
    {
...
function runAsync(db,sql, params,errMsg) {
  //console.log(sql);
  return new Promise((resolve, reject) => {
    // db.execute returns its own Promise, which we chain off of
    db.execute({
        sql:sql,
        args: params,
      })
      .then((result) => {
        // If db.execute is successful, we get the result object.
        // We extract the 'rows' and manually resolve our custom promise with those rows.
        resolve(result.rows); 
      })
      .catch((error) => {
        // If db.execute fails, we manually reject our custom promise with the error.
        reject(error); 
      });
  });
} 
/*async function runAsync(db,sql, params,errMsg) 
{
  console.log(sql, " from run async");
  if (params)
  {
    ...
    return insertResult = await db.execute(sql);
}*/
...
async function UpdateAccount(db,account,errMsg) 
{
  ...
  try
  {
    ...
    const result = await runAsync(txn,updateSql, [account.Code,account.Name,account.Tenant,account.Email,account.PhoneNo,account.AcType,account.ParentName,account.Address1,account.Address2,account.Address3,account.Id]);
...
async function AddAccount(db,account,errMsg) 
{
  ...
  try
  {
    ...
    const result = await runAsync(txn,updateSql, [account.Code,account.Name,account.Tenant,"",account.Email,account.PhoneNo,account.AcType,account.ParentName,account.Address1,account.Address2,account.Address3]);
  ...
  catch (err) 
  {
   ...
   try
   {
    if (forEditing)
    {
        ...
        const result = await runAsync(txn,deleteStmt);
    }
    ...
    let headerAccountNo = await GetAccountNo(txn,headerAccountName);
   ...
   //con sole.log("here1");
    for (let i = 0;i < transactions.length;++i)
    {
      ...
      let bodyAccountNo = await GetAccountNo(txn,bodyAccountName);
...
async function DeleteAccount(db,Id,errMsg) 
{
  try
  {
     sqlText = `Select VoucherNo From FAccounting Where (AccountDr = ${Id}) Or (AccountCr = ${Id})`
      const rows = await getData(db,sqlText);
      if (rows.length > 0 )
      {
        let err = {message:"Cannot delete account because vouchers are posted for this account."};
        throw err;
        return false;
      }
  }
  ...
  try 
  {
    ...
    const result = await runAsync(txn,updateSql, [Id]);
...
app.get('/api/data/transaction/:voucherno', async (req, res) => {
  ...
  try 
  {
    ...
    db = createClient(config);
    const { rows } = await db.execute(sStmt);
    res.json(rows);
...
app.get('/api/data/report/generalledger/:fromDate/:toDate/:accountGroup/:accountName', async (req, res) => {
  ...
  ///
      db = createClient(config);
      const { rows } = await db.execute(sStmtFinal);
      res.json(rows);
      return;
} 
catch (err)
...
/*async function getRowsFromDb() {
  ...
  try {
    ...
    const rows = await db.al l("SELECT * FROM users");
