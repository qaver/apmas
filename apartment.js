...
catch (err) 
{
    console.error("Error fetching html parameters :", err.message);
     ...
     const db = new sqlite3.Database('./AptAccounts.db', (err) =>
     { 

	      if (err) 
    	  {
 	         console.log('error conenecting to the SQLite database.');
           console.error(err.message);

    	  }
     	///else 
     ///{
     //      con sole.log('Connected to the SQLite database.');
     	///}
   });
 let bError =false;
try
  ...
  try 
 {
    ...
    // con sole.log(Code,Name,AcType,ParentName,Tenant,PhoneNo,Address1,Address2,Address3);
     ...
     { 
     	 ...
     	 /// else 
     	  //{
       //    cons ole.log('Connected to the SQLite database.');
      	//}
     });
     ...
     db.close((err) =>
     {
    	  if (err) 
        {
	     ...
	     /// else
	     ///   cons ole.log('Closed the database connection.');
     });
     if (!bReturnValue)
     {
	     /// cons ole.log(ErrMsg);
    ...
    {
        ///con sole.error(err);
        res.status(500).send({Id:-155,Name:'Edit Account:Server Error'});
    }
});
    ...
    // con sole.log(Code,Name,AcType,ParentName,Tenant,PhoneNo,Address1,Address2,Address3);
     ...
     db.close((err) =>
     {
    	    if (err) 
          {
	       ...
	       // else
	        ///   con sole.log('Closed the database connection.');
     });
     if (!bReturnValue)
     {
	        ///con sole.log(ErrMsg);
...
///
 ...
 {
    ///con sole.error(err);
    res.status(500).send({Id:-155,Name:'Get Accounts:Server Error.'+err.message});
  }
});
  ...
  try 
 {
  ...
  app.delete('/api/data/transaction/delete/:voucherNo', async (req, res) =>
  {
    try 
    {
      ...
      {
        //con sole.error(err);
        res.status(500).send({Id:-155,errMsg:'Delete voucher:Server Error.'});
      }
  });
...
///
 ...
 {
    ///con sole.error(err);
    res.status(500).send({Id:-155,Name:'General Ledger:Server Error.'+err.message});
  }
});
